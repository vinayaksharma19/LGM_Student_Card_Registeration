...
const getData = () => {
    ...
    info.url = document.getElementById('image').value;
    info.website = document.getElementById('website').value;
    info.gender = document.querySelector('input[name="M-F"]:checked').value;
    ...
    if (getLocalStorage === null) {
        ...
        cardDivArr.forEach((item, index) => {
            ...
            <div class="info">
                ...
                <button onclick="deleteData(${index})"><span class="material-symbols-outlined">
                delete
                </span></button>
            </div>
        </div>`;
